# TODO: add possibility to show the angle of current direction w.r. to -∇E
...
function default_scf_prompt()
    function prompt(info)
        if info.n_iter == 0
            ...
            info_out = [info.n_iter, info.E, log_ΔE, residual, NaN]
    ...
    # Select in "info" only the argmunents returned by the minimization procedure.
    clean(info) = (;energy=info.E, final_state=info.ζ, info.n_iter,
                    residual=norm(info.∇E)) # add solver name
    (; prompt, clean)
end
