...
@doc raw"""
        OLD: rohf_manifold_linesearch(ζ::State{T}, p::Matrix{T};
                                      E, ∇E, linesearch_type,
...
library.
"""
function rohf_manifold_linesearch(ζ::State{T}, p::Matrix{T};
                                  E, ∇E, linesearch_type,
                                  max_step = one(Float64),
                                  ) where {T<:Real}
    ...
    p_test = TangentVector(p, ζ)

    function f(step)
        ζ_next = retract(TangentVector(step .* p, ζ))
        rohf_energy!(ζ_next)
    end
    ...
    ζ_next = retract(TangentVector(α.*p, ζ))
