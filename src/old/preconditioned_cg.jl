...
"""
function preconditioned_CG_step(ΦT, Sm12, A, H, N_bds;
                                atom_info = [],
                                E_previous = zero(Float64),
                                previous_norm_∇E = zero(Float64),
                                ΦT_previous = zero(ΦT),
                                p_previous = ( zeros(N_bds[1],N_bds[2]),
                                              zeros(N_bds[1],N_bds[3]) ),
                                step = zero(Float64), restart_cg = true,
                                transport = "transport",
    ...
    @assert(transport ∈ ("proj", "transport"))
    ...
    if !(restart_cg)
        ...
        (transport == "transport") && (τ_p_prev = transport_MOs_same_dirs(p_previous,
                                                            step, ΦT_previous, ΦT, N_bds))
        (transport == "proj") && (τ_p_prev = proj_horizontal_tangent_space(ΦT,
                                                                       p_previous, N_bds))
        β = (norm_∇E^2)/(previous_norm_∇E^2)
        p_dir = p_dir .+ β .* τ_p_prev
    end
    ...
    # No linesearch if the direction is not a descent direction
    ...
    # restart if step = 0
    (step == 0) && ( return( (ΦT, ∇E_dir, E_previous, norm_∇E, cos_p_∇E, zero(Float64))) )

    ΦT_next, p_dir, E, norm_∇E, cos_p_∇E, step
end
...
""" 
function rohf_preconditioned_CG(Σ::ChemicalSystem;
                                ...
                                restart = false,
                                transport_type = "transport", max_step = one(Float64),
                                linesearch_type = BackTracking(order=3),
                                numerical_safety = 0.1,)
    ...
    p = (zeros(Nb,Nd), zeros(Nb,Ns))
    
    δE = zero(E); norm_∇E = zero(E);
    ...
    while( !(converged) & (iter < max_iter) )
        ...
        ΦT_next, p, E, norm_∇E, cos_p_∇E, step =  preconditioned_CG_step(ΦT, Sm12, A, H, N_bds,
                                                                         atom_info = atom_info,
        ...
        if !(verbosity == "none")
            ...
            println(@sprintf("%16.12f %16.12f %16.12f %16.12f  %1.5f %5i  | Preconditioned CG",
                             E, δE, norm_∇E, cos_p_∇E, step, iter))
            flush(stdout)
        end

        if( norm_∇E < cv_threshold )
            converged = true
        end
        ...
        if( (cos_p_∇E > -0.1) | (iter%(No*(Nb-No))==0) | (step==0) )
            restart_cg = true
        else
            restart_cg = false
        end
        ...
        Σ.cv_history = (converged, norm_∇E)
    end
